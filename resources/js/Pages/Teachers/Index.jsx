...
import CreateTeacher from '../../Components/Dashboard/Teacher/CreateTeacher';
import EditTeacher from '../../Components/Dashboard/Teacher/EditTeacher';
import { Inertia } from '@inertiajs/inertia';


export default function Index(props) {
    const { data: teachers, meta } = props.teachers;
    ...
    const destroyTeacher = () => {
        Inertia.delete(
            route('teachers.destroy', state.id), 
        ...
        <>
            <div className="container-fluid py-4">
                <Dialog trigger={addTrigger} title="Create New Student"> 
                    <CreateTeacher close={addCloseTrigger} />
                </Dialog>

                <Dialog trigger={updateTrigger} title={`Update Student: ${state.name}`}> 
                    <EditTeacher model={state} close={updateCloseTrigger} />
                </Dialog>
                ...
                <Dialog trigger={destroyTrigger} title={`Delete Stident: ${state.name}`}>
                    ...
                    <div className="modal-footer">
                        ...
                        <button type="button" className="btn bg-gradient-danger" onClick={destroyTeacher}>Delete</button>
                    </div>
                </Dialog>
                                            ...
                                            {teachers.map((teacher, index) => (
                                                <tr key={teacher.id}>
                                                    ...
                                                    <td className='text-left'>
                                                        <h6 className="mb-0 text-sm">{teacher.name}</h6>
                                                    </td>
                                                    <td className='text-left'>
                                                        <p className="text-sm font-weight-bold mb-0">{teacher.birth_date}</p>
                                                    </td>
                                                    <td className='text-left'>
                                                        <p className="text-sm font-weight-bold mb-0">{teacher.gender}</p>
                                                    </td>
                                                    <td className='text-left'>
                                                        <span className="text-xs font-weight-bold">{teacher.address}</span>
                                                    </td>
                                                    <td className="align-middle text-center" width="10%">
                                                        <div>
                                                            <button type="button" onClick={() => openUpdateDialog(teacher)} className="btn btn-vimeo btn-icon-only mx-2">
                                                                <span className="btn-inner--icon"><i className="fas fa-pencil-alt"></i></span>
                                                            </button>
                                                            <button type="button" onClick={() => openDestroyDialog(teacher)} className="btn btn-youtube btn-icon-only">
...
Index.layout = (page) => <Base key={page} children={page} title={"Manage teachers"} />;
